...
)
{
	...
	if (my_id == master_id)		cout << "Diagnostic parameters: ";
	i=1;
	para_file >> s >> diagnostic_procedure(i);
	
	if (my_id == master_id)
		cout << "Diagnostic procedure " <<  diagnostic_procedure(i) << " " ;
	
	while (diagnostic_procedure(i) < MY_MAX_INT)
	{
		i++;
		para_file >>  diagnostic_procedure(i);
		
		if (my_id == master_id)
			cout  << diagnostic_procedure(i) << "  " ;
	}
	...
	{	
		...
		cout << "force_U_switch (1 if present)"		<< switches(21) << endl;
		cout << "force_W_switch (1 if present)"		<< switches(22) << endl;
		cout << "force_T_switch (1 if present)"		<< switches(23) << endl;
		cout << "structure_fn_approx_switch (1 if yes) " << switches(24) << endl;
		cout << "EnertyTr_switch (1 if present)"		<< switches(25) << endl;
		cout << "globalvar_fftw_original_switch"		<< switches(26) << endl;
		cout << endl << endl;
	}	
	...
	for (i = 0; i < number_of_fields; i++) 
	{
		para_file >> hyper_diss_coefficient[i] >> hyper_diss_exponent[i]; 
		
		if (my_id == master_id)
			cout << hyper_diss_coefficient[i] << "    " << hyper_diss_exponent[i] << endl ;	
	}		
	...
	if (my_id == master_id)	
	{
		...
		cout << "Structure fn parameter: r-arraysize " << misc_output_para(6) << endl;
		cout << "Structure fn parameter: rmax/str_fn_rmax " << misc_output_para(7) << endl;
		cout << "Structure fn parameter:points to skip " << misc_output_para(8) << endl;
	}
