...
int RB_slip_main(string data_dir_name)
{
	...
	else if (globalvar_Pr_switch == "PRINFTY") 
	{
		if (globalvar_RB_Uscaling == "USMALL") 
		{
			diss_coefficients[0] = globalvar_Pr;             
			diss_coefficients[1]  = 1.0;			
		}
		else if (globalvar_RB_Uscaling == "ULARGE") 
		{
    ...
    cout << "diss" << diss_coefficients[0] << " " << diss_coefficients[1] << endl;
    ...
    if (my_id == 0)
        cout << U.Tnow << " " << U.CV_total_energy << " " << T.CS_total_energy << endl;
	...
	do
	{
		...
		U.CV_Compute_totalenergy();
		T.CS_Compute_totalenergy();
		if ( (my_id == 0) && (isnan(U.CV_total_energy) || isnan(T.CS_total_energy)) ) 
		{ 
