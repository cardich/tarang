...
) 
{
	...
	CV_shell_dissk=new Array<DP,1>(CV_shell_ek_size);  
	(*CV_shell_dissk) = 0.0;
	...
	if (CV_anisotropic_ring_switch == 1)
	{
		CV_ring_ek1  = new Array<DP,2>(CV_ring_ek_size, CV_no_sectors_spectrum+1);  
		(*CV_ring_ek1) = 0.0;
		
		CV_ring_ek2  = new Array<DP,2>(CV_ring_ek_size, CV_no_sectors_spectrum+1);  
		(*CV_ring_ek2) = 0.0;
		
		CV_ring_h1k  = new Array<DP,2>(CV_ring_ek_size, CV_no_sectors_spectrum+1);  
		(*CV_ring_h1k) = 0.0;
		
		CV_ring_dissk = new Array<DP,2>(CV_ring_ek_size, CV_no_sectors_spectrum+1); 
		(*CV_ring_dissk) = 0.0;
	}
	...
	if (CV_anisotropic_cylinder_switch == 1)
	{
		...
		CV_cylinder_ring_ek1 = new Array<DP,2>(CV_cylinder_shell_ek_size, 
										CV_no_cylinder_slabs_spectrum+1); 												
		
		CV_cylinder_ring_ek2 = new Array<DP,2>(CV_cylinder_shell_ek_size, 
										CV_no_cylinder_slabs_spectrum+1); 
										
		CV_cylinder_ring_dissk = new Array<DP,2>(CV_cylinder_shell_ek_size, 
										CV_no_cylinder_slabs_spectrum+1); 	
										
		CV_cylinder_ring_h1k = new Array<DP,2>(CV_cylinder_shell_ek_size, 
										CV_no_cylinder_slabs_spectrum+1); 																															
...
void CVF::CV_Compute_totalenergy()
{

	CV_total_E1 = Get_total_energy(CV_basis_type, CV_alias_switch, Ncv, *V1);
	CV_total_E2 = Get_total_energy(CV_basis_type, CV_alias_switch, Ncv, *V2);
	CV_total_E3 = Get_total_energy(CV_basis_type, CV_alias_switch, Ncv, *V3);
	
	CV_total_energy = CV_total_E1 + CV_total_E2 + CV_total_E3;
}


void CVF::CV_Compute_totaldiss()
{
...
void CVF::CV_Compute_totaldiss(Array<complx,3> W1, Array<complx,3> W2, Array<complx,3> W3)
{
	CV_total_dissipation_Hc  
	= 2 * (Get_total_Sn(CV_basis_type, CV_alias_switch, Ncv, *V1, W1, 2, CV_kfactor)  
		   + Get_total_Sn(CV_basis_type, CV_alias_switch, Ncv, *V2, W2, 2, CV_kfactor) 
		   + Get_total_Sn(CV_basis_type, CV_alias_switch, Ncv, *V3, W3, 2, CV_kfactor));	
}
...
void CVF::CV_Compute_ring_spectrum()
{
	if (CV_anisotropic_ring_switch == 1)
	{
		static Array<DP,2> temp(CV_ring_ek_size, CV_no_sectors_spectrum+1);  
		temp = 0.0;
...
)
{
	if (CV_anisotropic_ring_switch == 1)
	{
		static Array<DP,2> temp(CV_ring_ek_size, CV_no_sectors_spectrum+1);  temp = 0.0;
