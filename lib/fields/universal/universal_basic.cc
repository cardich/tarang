...
(
	...
	Array<complx,3> A, 
	DP factor, 
	DP hyper_factor, int hyper_exponent,
	DP kfactor[]
)
{
	if (basis_type == "FOUR")
		Array_exp_ksqr_FOUR(N, A, factor, hyper_factor, hyper_exponent, kfactor);
		
	else if (basis_type == "SCFT") 
		Array_exp_ksqr_SCFT(N, A, factor, hyper_factor, hyper_exponent, kfactor);
}
