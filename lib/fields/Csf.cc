...
) 
{
	...
	*F = 0.0;   // initialize
	...
	CS_shell_dissk = new Array<DP,1>(CS_shell_ek_size);  
	(*CS_shell_dissk)=0.0;

	if (CS_anisotropic_ring_switch == 1)
	{
		CS_ring_ek  = new Array<DP,2>(CS_ring_ek_size, CS_no_sectors_spectrum+1);  
		(*CS_ring_ek) = 0.0;
		
		CS_ring_dissk = new Array<DP,2>(CS_ring_ek_size, CS_no_sectors_spectrum+1); 
		(*CS_ring_dissk) = 0.0;
	}
	...
	if (CS_anisotropic_cylinder_switch == 1)
	{
		...
		CS_cylinder_ring_ek = new Array<DP,2>(CS_cylinder_shell_ek_size, 
									CS_no_cylinder_slabs_spectrum+1); 
									
		CS_cylinder_ring_dissk = new Array<DP,2>(CS_cylinder_shell_ek_size, 
									CS_no_cylinder_slabs_spectrum+1);	
...
void CSF::CS_Compute_totalenergy()
{
