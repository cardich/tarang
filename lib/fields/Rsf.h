...
public:
	...
	//! 1 if on; 0 if off
	int		RS_structure_fn_switch;
	
	//! 1 if on; 0 if off
	int		RS_planar_structure_fn_switch;
	
	int		RS_structure_fn_approx_switch;
	...
	DP		RS_xfactor[4];
	DP		RS_L[4];		// Box size
	DP		RS_Delta_x[4];	// Delta x along each direction
	
	
	
	// min q for structure function
	int	RS_structurefn_q_min;
	
	// max q for structure function
	int	RS_structurefn_q_max;
	
	int RS_structurefn_r_arraysize;
	
	//! Max r for structure function
	DP	RS_structurefn_r_max;
...
public:
	...
	*/
	...
	void RS_Output(ofstream& fileout, Array<complx,3> temp_array, string format);
	
	void RS_Output_transpose_order(ofstream& fileout, Array<complx,3> temp_r, string format);
	
	void RS_input(ifstream& file_in, Array<complx,3> temp_array, string format);
