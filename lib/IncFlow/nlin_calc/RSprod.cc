...
{	
	...
	if (globalvar_prog_kind == "KEPLERIAN") {
		...
		Yderiv_RSprod_VV(*V1r, *VF_temp);		
		*W.nlin2 = *W.nlin2 + complex<DP>(q_omega_t,0)*(*VF_temp);
		
		Yderiv_RSprod_VV(*W.V1r, *VF_temp);		
		*nlin2 = *nlin2 + complex<DP>(q_omega_t,0)*(*VF_temp);
		
		Yderiv_RSprod_VV(*V3r, *VF_temp);		
		*nlin3 = *nlin3 + complex<DP>(q_omega_t,0)*(*VF_temp);
		
		Yderiv_RSprod_VV(*W.V3r, *VF_temp);		
		*W.nlin3 = *W.nlin3 + complex<DP>(q_omega_t,0)*(*VF_temp);
	}
}
