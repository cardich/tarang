...
/*	DP q_omega_t = globalvar_Keplerian_q_shear* globalvar_Keplerian_omega* globalvar_Tnow;
	...
	if (globalvar_prog_kind == "KEPLERIAN") {
		Yderiv(basis_type, N, *V1, *VF_temp, kfactor, 0); 
		...
		for (int i2=0; i2<N[2]; i2++)
			for (int i3=0; i3<=N[3]/2; i3++)
			{
				if (abs((*F)(i1,i2,i3)) > MYEPS2)
					cout << "div: proc_id, l1, l2, l3, div " << q_omega_t << " " <<  my_id << " " << 
						i1 << "  " << i2 << " " << i3 << "    " << (*V1)(i1,i2,i3) << " " << (*V2)(i1,i2,i3) << "  " << abs((*F)(i1,i2,i3)) << endl;
			}
 ...
 */
	...
	Xderiv(basis_type, N, *V1, *VF_temp2, kfactor, 1);			// factor =1 (sin)
	
	if (globalvar_prog_kind == "KEPLERIAN") {
		Yderiv(basis_type, N, *V1, *VF_temp, kfactor, 0); 
		*VF_temp2 = *VF_temp2 + complex<DP>(q_omega_t,0)*(*VF_temp);
	}
	
	Yderiv(basis_type, N, *V2, *VF_temp, kfactor, 0);
	*VF_temp2 = *VF_temp2 + *VF_temp;
	...
	*VF_temp2 = *VF_temp2 + *VF_temp;
	...
	for (int i1=0; i1<local_N1; i1++)
		for (int i2=0; i2<N[2]; i2++)
			for (int i3=0; i3<=N[3]/2; i3++)
				{
				if (abs((*VF_temp2)(i1,i2,i3)) > MYEPS2)
					cout << "div: proc_id, l1, l2, l3, div " << q_omega_t << " " <<  my_id << " " << 
					i1 << "  " << i2 << " " << i3 << "    " << (*V1)(i1,i2,i3) << " " << (*V2)(i1,i2,i3) << "  " << abs((*VF_temp2)(i1,i2,i3)) << endl;
				}
