 ...
 *  @return  F(k) = alpha * V(k)  + beta(k) * Omega(k)
 */
void IncVF::Const_vector_field_alpha_beta(int kx, int ky, int kz, DP alpha, DP beta)
{
	...
	if (basis_type == "FOUR")	{
		if ( (lx >= 0) && (lx < local_N1) )		{
			...
			(*V1)(lx, ly, lz) = alpha * (*V1)(lx, ly, lz) 
								+ beta * vorticity(0);
			
			(*V2)(lx, ly, lz) = alpha * (*V2)(lx, ly, lz) 
								+ beta * vorticity(1);
			
			(*V3)(lx, ly, lz) = alpha * (*V3)(lx, ly, lz) 
								+ beta * vorticity(2);
		}								
	}								
		...
		if ( (lx >= 0) && (lx < local_N1) ) {
			...
			// in FOUR basis
			(*V1)(lx, ly, lz) = alpha * (-I) * (*V1)(lx, ly, lz) 
								+ beta * vorticity(0);
 ...
 *			of (-I) and (I).  Hence the conversion remains unchanged.
 */
void IncSF::Const_scalar_field_alpha(int kx, int ky, int kz, DP alpha)
{
