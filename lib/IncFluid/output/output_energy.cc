...
{	
	...
	T.CS_Compute_totalenergy();
	T.CS_Compute_totaldiss();
...
void IncFluid::Output_global_RB(IncSF& T)		
{
	...
	residual_T_energy = Get_total_energy_residual_SCFT("SCFT", N, *T.F);
	peclet_nu = globalvar_Ra*sqrt(2*Get_total_Sn_anis_SCFT("SCFT", N, *T.F, 6, kfactor));
	// For P=infty, Pe= R*sum_K(theta(k)^2 Kperp^2/K^6)
	
	nusselt_nu2 = 1+globalvar_Ra*2*Get_total_Sn_anis_SCFT("SCFT", N, *T.F, 4, kfactor);
...
void IncFluid::Output_global(IncVF& W)
{
	...
	W.CV_Compute_totaldiss();
		
	Hc = Get_cross_helicity(W);
	CV_Compute_totaldiss(*W.V1, *W.V2, *W.V3);
	...
	if (my_id == master_id) 
	{
		...
		DP total_diss = dissipation_coefficient*CV_total_dissipation;
		DP total_diss_Hc = ((dissipation_coefficient+W.dissipation_coefficient)/2)
									*CV_total_dissipation;
...
void IncFluid::Output_shell_spectrum(IncVF& W)
{
	...
	Compute_cross_helicity_shell_spectrum(W);
...
void IncFluid::Output_shell_spectrum(IncVF& W, IncSF &T)
{
	...
	Compute_cross_helicity_shell_spectrum(W);   // Hc(k) 
