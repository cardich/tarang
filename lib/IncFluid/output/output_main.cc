...
void IncFluid::Output_all_inloop()
{
	...
	if ((EnergyTr_switch == 1) && (Tnow >= Tflux_save_next))
	{
		Output_flux();			// 0 -> real_imag off
		Tflux_save_next += Tflux_save_interval;
	}	
			
	if ((EnergyTr_switch == 1) && (Tnow >= Tshell_to_shell_save_next))
	{
	...
	if ((EnergyTr_switch == 1) && (Tnow >= Tring_to_ring_save_next) 
				&& (CV_anisotropic_ring_switch == 1)) 
	{  
		Output_ring_to_ring(); 
	...
	if ((EnergyTr_switch == 1) && (Tnow >= Tcylinder_ring_to_ring_save_next) 	
				&& (CV_anisotropic_cylinder_switch == 1)) 
	{  
...
/*	DP q_omega_t = globalvar_Keplerian_q_shear* globalvar_Keplerian_omega* globalvar_Tnow;
	int kx=-9; 
	int ky= 1;
	int lx = Get_lx(basis_type, -9, N);
	int ly = Get_ly3D(basis_type, 1, N); */
	
	
	if (my_id == master_id)
		cout   << scientific << Tnow << " "  << CV_total_energy << " " << W.CV_total_energy << endl;
