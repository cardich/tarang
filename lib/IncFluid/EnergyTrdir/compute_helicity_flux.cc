...
// Fluid
void IncFluid::Compute_kinetic_helicity_flux()
{	
...
//
// Vector
void IncFluid::Compute_magnetic_helicity_flux(IncVF& W)
{
	...
	for (int sphere_index = 1; sphere_index <= no_spheres; sphere_index++) 
	{
		...
		(*flux_Whk)(sphere_index) = Prod_out_sphere_nlin_vector_potential(sphere_index, W)/2;	
		// flux_hm = W.grad U<. a>
		...
		(*flux_Whk)(sphere_index) += Prod_out_sphere_nlinV(sphere_index, W)/2;		
		// add (U x W<). B>
...
// Fluid 2D
void IncFluid::Compute_enstrophy_flux()
{	
...
// MHD 2D
void IncFluid::Compute_magnetic_enstrophy_flux(IncVF& W)
{	
	if (N[2] == 1)
	{
		(*flux_Whk) = 0.0;
		...
		for (int sphere_index = 1; sphere_index <= no_spheres; sphere_index++) 
		{
			...
			(*flux_Whk)(sphere_index) = Prod_out_sphere_nlin_vector_potential(sphere_index, W);	
			// (U.grad omega). omega>
		}
	}
}
