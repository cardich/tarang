 ...
 * @sa DP IncFluid::Get_Suu(TinyVector<DP,2> k, TinyVector<DP,2> p)
 *
 * @author  M. K. Verma
 * @version 4.0
 * @date Feb 2009
 *
 * @bugs  Lots of problems... All commented.. Also expensive.. left for postprocessing
 */
...
// u(p) to u(k)
DP IncFluid::Get_Suu(TinyVector<int,3> k, TinyVector<int,3> p)
{
/*	int lx, ly;
	...
	int data_size = 3*2;  // factor 2 due to complex to DP conversion
	MPI_Bcast(reinterpret_cast<DP*>(uk.data()), data_size, MPI_DP, 
	...
	data_size = 3*2;  // factor 2 due to complex to DP conversion
	MPI_Bcast(reinterpret_cast<DP*>(up.data()), data_size, MPI_DP, 
					master_id, MPI_COMM_WORLD);
	...
	data_size = 3*2;  // factor 2 due to complex to DP conversion
	MPI_Bcast(reinterpret_cast<DP*>(uq.data()), data_size, MPI_DP, 
					master_id, MPI_COMM_WORLD);
...
//*********************************************************************************************
// w(p) to w(k)
DP IncFluid::Get_Sww(IncVF& W, TinyVector<int,3> k, TinyVector<int,3> p)
{
...
/*	int lx, ly;
	...
	int data_size = 3*2;  // factor 2 due to complex to DP conversion
	MPI_Bcast(reinterpret_cast<DP*>(wk.data()), data_size, MPI_DP, 
					master_id, MPI_COMM_WORLD);
	...
	data_size = 3*2;  // factor 2 due to complex to DP conversion
	MPI_Bcast(reinterpret_cast<DP*>(wp.data()), data_size, MPI_DP, 
					master_id, MPI_COMM_WORLD);
...
//*********************************************************************************************
// u(p) to w(k)
DP IncFluid::Get_Swu(IncVF& W, TinyVector<int,3> k, TinyVector<int,3> p)
{
...
/*	int lx, ly;
	...
	data_size = 3*2;  // factor 2 due to complex to DP conversion
	MPI_Bcast(reinterpret_cast<DP*>(wq.data()), data_size, MPI_DP, 
					master_id, MPI_COMM_WORLD);
...
//*********************************************************************************************
// T(p) to T(k)
DP IncFluid::Get_Stt(IncSF& T, TinyVector<int,3> k, TinyVector<int,3> p)
{
...
/*	int lx, ly;
	...
	int data_size = 2;  // factor 2 due to complex to DP conversion
	MPI_Bcast(reinterpret_cast<DP*>(tk.data()), data_size, MPI_DP, 
					master_id, MPI_COMM_WORLD);
	...
	data_size = 2;  // factor 2 due to complex to DP conversion
	MPI_Bcast(reinterpret_cast<DP*>(tp.data()), data_size, MPI_DP, 
					master_id, MPI_COMM_WORLD);
 ...
 */
void IncFluid::Compute_Skpq(IncSF& T)
{
/*	no_of_Skpq_triads=1;
 ...
 */
void IncFluid::Compute_Skpq(IncVF& W)
{
/*	no_of_Skpq_triads=1;
 ...
 * @return (*S_SF_array)(i)		= Stt(i) for all i triads 
 */
void IncFluid::Compute_Skpq(IncVF& W, IncSF& T)
{
/*	no_of_Skpq_triads=1;
