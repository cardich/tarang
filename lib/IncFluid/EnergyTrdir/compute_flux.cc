...
void IncFluid::Compute_flux(IncSF& T)
{
...
void IncFluid::Compute_flux_scalar(IncSF& T)
{
...
//
// RB convection
void IncFluid::Compute_flux_RB(IncSF& T)
{
...
//
// Vector
void IncFluid::Compute_flux(IncVF& W)
{
	...
	{		
		...
		(*flux_VF_Uin_Wout)(sphere_index) = Prod_out_sphere_nlinV(sphere_index, W);	
		// (W.graad U<). W>
		
		(*flux_VF_Uin_Win)(sphere_index) = Prod_in_sphere_nlinV(sphere_index, W);		
		// (W.graad U<). W<
	}
	...
	{	
		...
		(*flux_VF_Win_Wout)(sphere_index) = -Prod_out_sphere_nlinV(sphere_index, W);		
		// -(U.graad W<). W>
	...
	{	
		...
		(*flux_VF_Win_Uout)(sphere_index) = Prod_out_sphere_nlinV(sphere_index);	
		//  (U.graad W<). U>
	...
	{		
		...
		(*flux_VF_Uout_Wout)(sphere_index) = Prod_out_sphere_nlinV(sphere_index, W);	
		// (W.graad U>). W>
	}
	...
	{		
		...
		(*flux_Elsasser_plus)(sphere_index) = -Prod_out_sphere_nlinV(sphere_index);	
		// (Zm.graad Zp<). Zp>
	...
	for (int sphere_index = 1; sphere_index <= no_spheres; sphere_index++) 
	{
		...
		(*flux_Elsasser_minus)(sphere_index) = -Prod_out_sphere_nlinV(sphere_index, W);	
		// (Zp.graad Zm<). Zm>
...
void IncFluid::Compute_flux(IncVF& W, IncSF& T)
{
