...
class IncFluid: public IncVF,public EnergyTr, public Time, public Tarang_HDF5_IO  // Vectorfield
{
public:
	...
	int				EnergyTr_switch;				// 1 if EnergyTr is on				
...
public:
	...
	void Compute_force_const_energy_supply();
	void Compute_force_const_energy_supply(IncSF& T);
	void Compute_force_const_energy_supply(IncVF& W);
	void Compute_force_const_energy_supply(IncVF& W, IncSF& T);
	...
	void Compute_force_const_energy();			
	void Compute_force_const_energy(IncSF& T);
	void Compute_force_const_energy(IncVF& W);						
	void Compute_force_const_energy(IncVF& W, IncSF& T);
	...
	void Compute_force_Liquid_metal(IncSF& T);
	void Compute_force_Liquid_metal_const_energy_supply();
	void Compute_force_Ekman_friction();
	void Compute_force_Ekman_friction_const_energy_supply();
	...
	void Compute_force_Kolmogorov_flow();
    void Compute_force_Keplerian(IncVF& W);
	...
	void Init_cond_dynamo_full_velocity_field(IncVF& W);
	
	void Init_cond_real_field();
	void Init_cond_real_field(IncSF& T);
	void Init_cond_real_field_scalar(IncSF& T);
	void Init_cond_real_field_RB(IncSF& T);
	void Init_cond_real_field(IncVF& W);
	void Init_cond_real_field(IncVF& W, IncSF& T);
