...
void IncFluid::Read_init_cond()
{
	switch (field_input_proc) 
	{
		...
		case (1001):Init_cond_real_field(); break; 	
	}
...
void IncFluid::Read_init_cond(IncSF& T)
{
	...
	switch (field_input_proc) 
	{
		...
		case (1001):Init_cond_real_field(T); break;	
	}
...
void IncFluid::Read_init_cond(IncVF& W)
{
	switch (field_input_proc)
	{
		...
		case (102) : Init_cond_dynamo_full_velocity_field(W); break;	
		// Full velocity field for a given Re, then introduce random B field in a band
			
		case (1001):Init_cond_real_field(W); break;	
	}
...
void IncFluid::Read_init_cond(IncVF& W, IncSF& T)
{
	switch (field_input_proc) 
	{
		...
		case (1001):Init_cond_real_field(W, T); break;	
	}
