...
(
	 ...
	 string	force_string_parameters[]
		...
		IncVF(N, string_switches, switches, force_U_switch, prog_kfactor, 
				diss_coefficient, hyper_diss_coefficient,  hyper_diss_exponent, misc_output_para),
		EnergyTr(N, string_switches, switches, prog_kfactor,ET_parameters, 
					ET_shell_radii_sector_array),
		Time(time_para, time_save_interval), 
		Tarang_HDF5_IO(string_switches[0], N)
{
...
///	
	...
	for (int i=1; i<=3; i++)
		N_out_reduced[i] = misc_output_para(8+i);
