...
//	if (my_id == master_id)		
//		cout << "force*v= " << sum(*shell_spectrum_force_Vk) << endl;
...
{
	if (T.force_switch == 0)
		Compute_force_const_energy_helicity_supply();
...
void IncFluid::Compute_force_const_energy_helicity_supply(IncVF& W, IncSF& T)
{
	...
	if ((W.force_switch == 0) && (T.force_switch == 0))
		...
		Compute_force_const_energy_helicity_supply(W);
