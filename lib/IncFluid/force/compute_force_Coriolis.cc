...
void IncFluid::Compute_force_Keplerian(IncVF& W)
{
	DP two_omega =  2*globalvar_Keplerian_omega;
	
	if (force_switch == 1) {
		*Force1 = two_omega*(*V2);
		*Force2 = (globalvar_Keplerian_q_shear-2)*globalvar_Keplerian_omega*(*V1);
		*Force3 = 0.0;
	}
	
	if (W.force_switch == 1) {
		*W.Force1 = 0.0;
		*W.Force2 = -globalvar_Keplerian_q_shear*globalvar_Keplerian_omega*(*W.V1);
		*W.Force3 = 0.0;
	}
