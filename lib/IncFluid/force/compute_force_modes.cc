...
void  IncFluid::Compute_force_given_modes_SIMPLE()
{
	...
	if ((force_switch == 1) && (is_force_field_para_read == 0))
	{
		...
		MPI_Bcast( reinterpret_cast<DP*>((*force_field_modes_F_array).data()), data_size, 
									MPI_DP, master_id, MPI_COMM_WORLD);	
...
void IncFluid::Compute_force_given_modes_SIMPLE(IncSF& T)
{
	...
	if (T.force_switch == 0)
		Compute_force_given_modes_SIMPLE();
	
	else if ((T.force_switch == 1) && (is_force_field_para_read == 0))
	{
		Input_force_prefix(force_field_in_file);
...
void  IncFluid::Compute_force_given_modes_SIMPLE(IncVF& W)
{
	...
	else if ((W.force_switch == 1) && (is_force_field_para_read == 0))
	{
		Input_force_prefix(force_field_in_file);
...
void  IncFluid::Compute_force_given_modes_SIMPLE(IncVF& W, IncSF& T)
{
	if ((W.force_switch == 0) && (T.force_switch == 0))
		...
		Compute_force_given_modes_SIMPLE(W);
	
	else if ((W.force_switch == 1) && (T.force_switch == 1) && (is_force_field_para_read == 0))
	{
		Input_force_prefix(force_field_in_file);
