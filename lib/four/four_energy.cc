...
DP Get_local_Sn_FOUR(string alias_switch, int N[], Array<complx,3> A, DP n, DP kfactor[])
{
	...
	for (int l1=0; l1<local_N1; l1++)			
		for (int l2=0; l2<N[2]; l2++) 
			for (int l3=0; l3<=maxN3; l3++) 
			{
				...
				if (kkmag <= kkmax)
					Sn += Multiplicity_factor_FOUR(l1,l2,l3,N) * my_pow(kkmag,n) 
														   * pow2(abs(A(l1,l2,l3)));
			}
	...
	if ((my_id == master_id) && (n==0))
			Sn += -Multiplicity_factor_FOUR(0, 0, 0, N) * pow2(abs(A(0,0,0)));		
...
)
{
	...
	// Subtract the contribution from the origin only for n=0.
	if ((my_id == master_id) && (n==0))
		Sn += -Multiplicity_factor_FOUR(0, 0, 0, N) * real(A(0,0,0)*conj(B(0,0,0)));
...
{
	DP local_total;
	local_total = Get_local_Sn_FOUR(alias_switch, N, A, B, n, kfactor);
...
)
{
	...
	DP entropy;
	MPI_Reduce(&local_entropy, &entropy, 1, MPI_DP, MPI_SUM, 
...
)
{
	...
	Compute_local_cylinder_ring_spectrum_FOUR(alias_switch, N, Ax, Ay, Az, Bx, By, Bz, 
									cylinder_kpll_array, n, local_S1k, local_S2k, kfactor);
...
)
{
	...
	Compute_local_cylinder_ring_spectrum_FOUR(alias_switch, N, F, cylinder_kpll_array, n, 
										local_Sk, kfactor);
...
)
{
	...
	Compute_local_cylinder_ring_spectrum_FOUR(alias_switch, N, F, G, cylinder_kpll_array, n, 
										local_Sk, kfactor);
...
)
{
	...
	Compute_local_cylinder_ring_spectrum_helicity_FOUR(alias_switch, N, Ax, Ay, Az, cylinder_kpll_array, 
												local_H1k, kfactor);
