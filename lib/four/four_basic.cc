...
{	
    ...
    if (globalvar_prog_kind == "KEPLERIAN") {
        for (int l1 = 0; l1 < local_N1; l1++) {
            ...
            for (int l2=0; l2<=N[2]/2; l2++) {
                ...
                for (int l3=0; l3<=N[3]/2; l3++) {
                    Ksqr = (pow2(Kx) + pow2(Ky) + pow2(l3*kfactor[3]));
                    mu_sqr = Ksqr + pow2(q_omega_t*Ky) + 2*q_omega_t*Kx*Ky;
                    
                    A(l1, l2, l3) *= exp(factor*mu_sqr); 
                }
            }
        ...
        for (int l1 = 0; l1 < local_N1; l1++) {
            ...
            for (int l2=0; l2<=N[2]/2; l2++) 
                for (int l3=0; l3<=N[3]/2; l3++) {
					Ksqr = (pow2(Kx) + pow2(l2*kfactor[2]) + pow2(l3*kfactor[3]));
                    
                    A(l1, l2, l3) *= exp(factor*Ksqr); 
                }
            
            if (N[2] >1)
                for (int l2=N[2]/2+1; l2<N[2]; l2++) 
                    for (int l3=0; l3<=N[3]/2; l3++) {
                        Ksqr = (pow2(Kx) + pow2((l2-N[2])*kfactor[2]) 
                                 + pow2(l3*kfactor[3]));
...
/*
 ...
 // Saves exp(-nu k^2 dt), but dt will be fixed.. that sacrifices efficiency.
 Also we need to save separately fpr MHD, scalar. 
 
void Array_exp_ksqr_FOUR(int N[], Array<complx,3> A, DP factor, DP kfactor[])
{	
	int k1;
	
	static Array<DP,1> expkkx2(local_N1);
	static Array<DP,1> expkky2(N[2]);
	static Array<DP,1> expkkz2(N[3]/2+1);
	
	if ((globalvar_exp_calc_done_switch == 0) || (globalvar_LES_switch == 1)) {
		for (int l1 = 0; l1 < local_N1; l1++) {
			k1 = Get_kx_FOUR(l1, N);
			expkkx2(l1) = exp(factor*pow2(k1*kfactor[1]));
		}
		
		for (int l2=0; l2<=N[2]/2; l2++) 
			expkky2(l2) = exp(factor*pow2(l2*kfactor[2]));
		
		if (N[2] >1)
			for (int l2=N[2]/2+1; l2<N[2]; l2++) 
				expkky2(l2) = exp(factor*pow2((l2-N[2])*kfactor[2]));
		
		for (int l3=0; l3<=N[3]/2; l3++)
			expkkz2(l3) = exp(factor*pow2(l3*kfactor[3]));
		
		globalvar_exp_calc_done_switch = 1;
	}
	
	for (int l1 = 0; l1 < local_N1; l1++) 
		for (int l2=0; l2<N[2]; l2++) 
			for (int l3=0; l3<=N[3]/2; l3++)
				A(l1, l2, l3) *= (expkkx2(l1)*expkky2(l2)*expkkz2(l3));
}
...
void Array_exp_ksqr_FOUR(int N[], Array<complx,3> A, DP factor, 
						 DP hyper_factor, int hyper_exponent, DP kfactor[])
{	
	int Kx, Ky, Kz;
	DP Ksqr, mu_sqr;
        ...
        for (int l1 = 0; l1 < local_N1; l1++) {
            ...
            for (int l2=0; l2<=N[2]/2; l2++) {
                ...
                for (int l3=0; l3<=N[3]/2; l3++) {
                    ...
                    A(l1, l2, l3) *= exp((factor+hyper_factor*Kpownm2)*mu_sqr); 
                }
            }
		...
		for (int l1 = 0; l1 < local_N1; l1++) {
			...
			for (int l2=0; l2<=N[2]/2; l2++) {
				...
				for (int l3=0; l3<=N[3]/2; l3++) {
					...
					A(l1, l2, l3) *= exp((factor+hyper_factor*Kpownm2)* Ksqr); 
				}
			}
