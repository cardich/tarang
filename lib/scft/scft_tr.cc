...
void Init_fftw_plan_SCFT(int N[], Array<complx,3> A)
{
	if (N[2] > 1) {
		Array<DP,1> temp_row(N[1]);							// temp row for SIN/COS transform
		Array<complx,2> temp_plane(N[2],(N[3]/2)+1);		// temp Plane for fourier transform
			
		sintr_plan_SCFT = FFTW_PLAN_R2R_1D_DP(N[1],(DP*)(temp_row.data()), (DP*)(temp_row.data()), 
								FFTW_RODFT10, FFTW_PLAN_FLAG);
		costr_plan_SCFT = FFTW_PLAN_R2R_1D_DP(N[1],(DP*)(temp_row.data()), (DP*)(temp_row.data()), 
								FFTW_REDFT10, FFTW_PLAN_FLAG); 
	  
		isintr_plan_SCFT = FFTW_PLAN_R2R_1D_DP(N[1],(DP*)(temp_row.data()), (DP*)(temp_row.data()), 
								FFTW_RODFT01, FFTW_PLAN_FLAG);
		icostr_plan_SCFT = FFTW_PLAN_R2R_1D_DP(N[1],(DP*)(temp_row.data()), (DP*)(temp_row.data()), 
								FFTW_REDFT01, FFTW_PLAN_FLAG);  
	  
		r2c_plan_SCFT = FFTW_PLAN_DFT_R2C_2D_DP(N[2],N[3],(DP*)(temp_plane.data()), 
								 (FFTW_COMPLEX_DP*)(temp_plane.data()),  FFTW_PLAN_FLAG);
		c2r_plan_SCFT = FFTW_PLAN_DFT_C2R_2D_DP(N[2],N[3],(FFTW_COMPLEX_DP*)(temp_plane.data()), 
								 (DP*)(temp_plane.data()),FFTW_PLAN_FLAG);  
	}
	
	else if (N[2] ==1) {
		Array<DP,1> temp_row(N[1]);							// temp row for SIN/COS transform
		Array<complx,1> temp_column_2d((N[3]/2)+1);			// For 2D 
		...
		r2c_1d_plan_SCFT = FFTW_PLAN_DFT_R2C_1D_DP(N[3], (DP*)(temp_column_2d.data()), 
											(FFTW_COMPLEX_DP*)(temp_column_2d.data()), FFTW_PLAN_FLAG);
		c2r_1d_plan_SCFT = FFTW_PLAN_DFT_C2R_1D_DP(N[3],(FFTW_COMPLEX_DP*)(temp_column_2d.data()), 
											(DP*)(temp_column_2d.data()), FFTW_PLAN_FLAG); 
	}
}
...
void Sintr_row_SCFT(FFTW_PLAN_DP sintr_plan_SCFT, int N[], Array<DP,1> Row)
{
	FFTW_EXECUTE_R2R_DP(sintr_plan_SCFT, reinterpret_cast<DP*>(Row.data()),
			reinterpret_cast<DP*>(Row.data()));  
}

void Costr_row_SCFT(FFTW_PLAN_DP costr_plan_SCFT, int N[], Array<DP,1> Row)
{
	FFTW_EXECUTE_R2R_DP(costr_plan_SCFT, reinterpret_cast<DP*>(Row.data()),
			 reinterpret_cast<DP*>(Row.data())); 
}

void ISintr_row_SCFT(FFTW_PLAN_DP isintr_plan_SCFT, int N[], Array<DP,1> Row)
{
	FFTW_EXECUTE_R2R_DP(isintr_plan_SCFT, reinterpret_cast<DP*>(Row.data()),
			 reinterpret_cast<DP*>(Row.data()));  
}			 

void ICostr_row_SCFT(FFTW_PLAN_DP icostr_plan_SCFT, int N[], Array<DP,1> Row)
{			 
	FFTW_EXECUTE_R2R_DP(icostr_plan_SCFT, reinterpret_cast<DP*>(Row.data()),
			reinterpret_cast<DP*>(Row.data()));  		 
}	


void FT_Plane_SCFT(FFTW_PLAN_DP r2c_plan_SCFT, int N[], Array<complx,2> Plane)	
{						
	FFTW_EXECUTE_DFT_R2C_DP(r2c_plan_SCFT, reinterpret_cast<DP*>(Plane.data()), 
                           reinterpret_cast<FFTW_COMPLEX_DP*>(Plane.data()));	
}	

	
void IFT_Plane_SCFT(FFTW_PLAN_DP c2r_plan_SCFT, int N[], Array<complx,2> Plane)
{				
	FFTW_EXECUTE_DFT_C2R_DP(c2r_plan_SCFT, reinterpret_cast<FFTW_COMPLEX_DP*>(Plane.data()), 
                         reinterpret_cast<DP*>(Plane.data()));
}	

void FT_column_1d_SCFT(FFTW_PLAN_DP r2c_1d_plan_SCFT, int N[], Array<complx,1> temp_column_2d)	
{						
	FFTW_EXECUTE_DFT_R2C_DP(r2c_1d_plan_SCFT, reinterpret_cast<DP*>(temp_column_2d.data()), 
						 reinterpret_cast<FFTW_COMPLEX_DP*>(temp_column_2d.data()));	
}	


void IFT_column_1d_SCFT(FFTW_PLAN_DP c2r_1d_plan_SCFT, int N[], Array<complx,1> temp_column_2d)
{				
	FFTW_EXECUTE_DFT_C2R_DP(c2r_1d_plan_SCFT, 
						 reinterpret_cast<FFTW_COMPLEX_DP*>(temp_column_2d.data()), 
...
void ArrayShiftRight_SCFT(int N[], Array<complx,3> A)
{
	...
	{												
		...
		MPI_Send( reinterpret_cast<DP*>(temp_plane.data()), data_size, MPI_DP, 
	...
	if (my_id > 0) 
	{
		MPI_Recv( reinterpret_cast<DP*>(temp_plane.data()), data_size, 
						MPI_DP,my_id-1, tag, MPI_COMM_WORLD, &status );							
...
void ArrayShiftLeft_SCFT(int N[], Array<complx,3> A)
{
	...
	// All the procs except the last one send the first plane to the next-left processor
	if (my_id > 0) 
	{												
...
(
	FFTW_PLAN_DP sintr_plan_SCFT, 
	FFTW_PLAN_DP r2c_plan_SCFT, 
	FFTW_PLAN_DP r2c_1d_plan_SCFT,
	int N[], 
	Array<complx,3> Atr, 
	Array<complx,3> A
)
{
	if (N[2] > 1)
	{
		...
		Zero_pad_lastplane_SCFT(N, Atr);							 // Zero_pad the row=N[3]/2
	...
	else if (N[2] == 1)
	{
		...
		// Array Atr is transposed N3/2+1, N2, local_N1
		
		// First zero pad Atr(N[3]/2+1,0,:)
		if (my_id == numprocs-1) 
			Atr(local_N3, 0, Range::all()) = 0.0;	
		
		
		// Sin transform along x for all z of Atr
		for (l1=0; l1<local_N3; l1++) {
...
(
	FFTW_PLAN_DP isintr_plan_SCFT, 
	FFTW_PLAN_DP c2r_plan_SCFT, 
	FFTW_PLAN_DP c2r_1d_plan_SCFT,
	int N[], 
	Array<complx,3> A, 
	Array<complx,3> Atr
)  
{
	...
	else if (N[2] == 1)
	{
		...
		// Array Atr is transposed N3/2+1, 0, local_N1
		// ISin transform along x for all z of Atr
		for (l1=0; l1 < local_N3; l1++)	{        
			temp_col = real(Atr(l1, 0, Range::all())); 
			ISintr_row_SCFT(isintr_plan_SCFT, N, temp_col);  
			real(Atr(l1, 0, Range::all())) = temp_col;
		...
		if (my_id == numprocs-1) 
			Atr(local_N3, 0, Range::all()) = 0.0;				// zero_pad  
	}
}
...
void  Xderiv_Sin_SCFT(int N[], Array<complx,3> A, Array<complx,3> B, DP kfactor[])
{	
	int k1;
...
void Xderiv_Cos_SCFT(int N[], Array<complx,3> A, Array<complx,3> B, DP kfactor[])
{
	int k1;
...
void Yderiv_SCFT(int N[], Array<complx,3> A, Array<complx,3> B, DP kfactor[])
{
	secondIndex i2;
	...
	B(Range::all(),Range(N[2]/2+1,N[2]-1),Range::all()) = 
           complex<DP>(0,kfactor[2])*((DP) 1.0*(i2+1-N[2]/2))
...
void Zderiv_SCFT(int N[], Array<complx,3> A, Array<complx,3> B, DP kfactor[])
{
	thirdIndex i3;
