	...
	for (int l1 = 0; l1 < local_N1; l1++) 
	{
		...
		if (N[2] >1)
			for (int l2=N[2]/2+1; l2<N[2]; l2++) 
				for (int l3=0; l3<=N[3]/2; l3++) {
					kksqr = (pow2(k1*kfactor[1]) + pow2((l2-N[2])*kfactor[2]) 
							 + pow2(l3*kfactor[3]));
...
void Array_exp_ksqr_SCFT(int N[], Array<complx,3> A, DP factor, 
						 DP hyper_factor, int hyper_exponent, DP kfactor[])
{
	int k1;
	DP kksqr;
	DP kkpownm2;	// K^{q-2} where q = hyper_exponent
		...
		for (int l2=0; l2<=N[2]/2; l2++) 
			for (int l3=0; l3<=N[3]/2; l3++) {
				...
				if (hyper_exponent == 4) 
					...
					kkpownm2 = my_pow(kksqr,(hyper_exponent-2)/2);
				
				A(l1, l2, l3) *= exp((factor+hyper_factor*kkpownm2)* kksqr);
			}
